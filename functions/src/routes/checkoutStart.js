// functions/src/routes/checkoutStart.js
...
async function createPendingPrepaidOrder({ db, decoded, reqBody }) {
  ...
  const result = await db.runTransaction(async (tx) => {
    ...
    tx.set(orderRef.collection("events").doc(), {
      type: "ORDER_CREATED",
      source: "firebase-functions/checkout-start",
      actor: {
        type: "customer",
        uid,
      },
  ...
  try {
    ...
    const razorpayOrder = await createRazorpayOrder({
      ...
      notes: {
        ...
        source: "daylybuy_mobile_app_firebase_functions",
      },
    });
          ...
          .set(
            {
              ...
              "payment.failureReason":
                error?.message || "Failed to start Razorpay checkout",
